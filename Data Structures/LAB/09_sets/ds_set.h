...
public:
  ...
  iterator iterative_find(const T& key_value) {return iterative_find(key_value, root_); };
...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    if (old_root != nullptr) {
        ...
        new_node->value = old_root->value;
        new_node->left = copy_tree(old_root->left);
        new_node->right = copy_tree(old_root->right);
    } else return nullptr;
  ...
  iterator iterative_find(const T& key_value, TreeNode<T>* p) {
