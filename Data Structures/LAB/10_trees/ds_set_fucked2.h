...
public:
  ...
  tree_iterator<T> & operator--() {
    if (ptr_->left != NULL) { // find the leftmost child of the right node
      ptr_ = ptr_->left;
...
public:
  ...
  std::pair< iterator, bool > insert(T const& key_value) {
    std::pair< iterator, bool > itr = insert(key_value, root_, NULL);
  ...
  int erase(T const& key_value) {
    int success = erase(key_value, root_);
...
private:
  ...
  std::pair<iterator,bool> insert(const T& key_value, TreeNode<T>*& p, TreeNode<T>* the_parent) {
    if (!tail_) {
        tail_ = new TreeNode<T>();
        dummytail_ = new TreeNode<T>();

        dummytail_->parent = tail_;
        tail_->right = dummytail_;
    }
