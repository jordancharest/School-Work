...
public:
    ...
    // MODIFIERS
    void set_position(intptr_t x, intptr_t y){
        Xpos_ = x;
        Ypos_ = y;
    }

    void set_speed(intptr_t s){
        speed_ = s;
    }
...
// PRINT PACE ==============================================================================
void print_pace(const intptr_t &speed){
    ...
    std::stringstream ss;
    ss << minutes << ":" << filler << (int) seconds;
    std::string paceString = ss.str();
    std::cout << paceString;
}

// PRINT ===================================================================================
void print(GPSData *GPSDataArray, const intptr_t &numData){
    ...
    for(intptr_t j=0; j<columns; j++){
        std::cout << std::setw(10) << std::fixed << std::right << headers[j];
    }
    std::cout << std::endl;


    for (intptr_t i=0; i<numData; i++){
...
double distance(GPSData *data, intptr_t n, intptr_t &avg_feet_per_minute){
    ...
    thestack.set_label((intptr_t*)&data,"n");
    thestack.set_label((intptr_t*)&n,"DATA ARRAY");
    thestack.set_label((intptr_t*)&avg_feet_per_minute,"AVG FEET PER MINUTE REF.");

    avg_feet_per_minute = 0;

    intptr_t xCurrent, xLast, yCurrent, yLast;
    ...
    for (intptr_t i = 1; i<n; i++){
        ...
        data[i].set_speed( intptr_t (speed));
...
intptr_t main(intptr_t argc, char** argv) {
  ...
  GPSData input[count];
  //thestack.set_label((intptr_t*)&input,"Input Array");
  ...
  //thestack.set_label((intptr_t*)original_avg_speed,"AVG FEET PER MINUTE");
  ...
  //thestack.set_label((intptr_t*)&filtered,"Filtered Array");
