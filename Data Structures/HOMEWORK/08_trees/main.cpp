...
void student_tests() {
  ...
  std::vector< std::pair<Point<long>,char> > simple_points;
  simple_points.push_back(std::make_pair(Point<long>(50,30), 'A'));
  simple_points.push_back(std::make_pair(Point<long>(10,5), 'B'));
  simple_points.push_back(std::make_pair(Point<long>(3,4), 'C'));
  simple_points.push_back(std::make_pair(Point<long>(11,7), 'D'));
  simple_points.push_back(std::make_pair(Point<long>(31,6), 'E'));
  simple_points.push_back(std::make_pair(Point<long>(40,13), 'F'));
  simple_points.push_back(std::make_pair(Point<long>(12,20), 'G'));
  simple_points.push_back(std::make_pair(Point<long>(4,8), 'H'));
  simple_points.push_back(std::make_pair(Point<long>(14,9), 'I'));
  simple_points.push_back(std::make_pair(Point<long>(25,15), 'J'));
  simple_points.push_back(std::make_pair(Point<long>(35,21), 'K'));
  simple_points.push_back(std::make_pair(Point<long>(26,17), 'L'));



  // --------------------------------------------------------
  // the quad tree data structure starts out empty
  QuadTree<long,char> simple;
  assert (simple.size() == 0);
  // an empty tree has height == -1
  assert (simple.height() == -1);
  // plot the structure with with these dimensions (width=40,height=20)
  ...
  for (unsigned int i = 0; i < simple_points.size(); i++) {
    ...
    if (i == 0) {
      ...
      assert (simple.height() == 0);
      // check that the newly inserted element can be found
      QuadTree<long,char>::iterator itr = simple.find(50,30);
      assert (itr != simple.end());
      // read the label & coordinates from the iterator
      assert (itr.getLabel() == 'A');
      // dereference the iterator to get the point
      const Point<long> &pt = *itr;
      assert (pt.x == 50);
      assert (pt.y == 30);
    }
  }
  ...
  // More tests on the copy constructor and assignment operator
  QuadTree<long,char> copy_tree (simple);

  std::cout << "\nSHOULD BE IDENTICAL TO SIMPLE TREE\n";
  copy_tree.plot(60,40);
  copy_tree.print_sideways();

  copy_tree.insert(Point<long>(41,29), 'M');

  std::cout << "\nINSERTED M\n Original Size: " << simple.size() << "\n Copy Size: " << copy_tree.size() << "\n";
  simple.print_sideways();
  std::cout << "\nCopy with M inserted\n";
  copy_tree.print_sideways();



  QuadTree<long,char> new_tree = simple;
  std::cout << "\n\n\nTesting Increment and Decrement iterator operators\n";
  QuadTree<long,char>::bf_iterator bf_itr = new_tree.bf_begin();


  std::cout << "\nBreadth First Iterator: These should mirror each other\nas the iterator travels down the tree and back up";
  int level = -1;
  for (int i = 0; i < new_tree.size()-1; ++i) {
    int depth = bf_itr.getDepth();
    if (level != depth) {
      level = depth;
      // starting a new level!
  ...
  QuadTree<long,char>::iterator itr = new_tree.begin();
  std::cout << "\n\nDepth First Iterator: These should mirror each other\nas the iterator travels down the tree and back up\n";
  for (int i = 0; i < new_tree.size()-1; ++i) {
    int depth = itr.getDepth();
    std::cout << std::string(depth*2,' ') << itr.getLabel() << " " << *itr << std::endl;
    ++itr;
  }
